...
const AboutContantSection = ({ PageName }) => {
  ...
  useEffect(() => {
    ...
    if (progressBar1Ref.current) observer.observe(progressBar1Ref.current);
    if (progressBar2Ref.current) observer.observe(progressBar2Ref.current);
    if (progressBar3Ref.current) observer.observe(progressBar3Ref.current);

    // Cleanup observer on component unmount
    return () => {
      if (progressBar1Ref.current) observer.unobserve(progressBar1Ref.current);
      if (progressBar2Ref.current) observer.unobserve(progressBar2Ref.current);
      if (progressBar3Ref.current) observer.unobserve(progressBar3Ref.current);
    };
  }, []);
      ...
      <div className="container py-5 px-lg-5">
        <div className="row g-5 align-items-center">
          <div className="col-lg-6 wow fadeInUp mb-5" data-wow-delay="0.1s">
            ...
            <Link href="/about" className="btn btn-primary py-sm-3 px-sm-5 rounded-pill mt-3">Read More</Link>
          </div>
